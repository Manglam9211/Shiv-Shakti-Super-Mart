...
// FIXED: Opened 50MB Highway for Heavy Mobile High-Quality Photos
...
mongoose.connect(mongoURI)
  .then(() => console.log("Database Sync Active with 50MB Payload Engine"))
...
// 🚀 BULLETPROOF BASE64 DIRECT TRANSMISSION ROUTE WITH AUTOMATIC CLEANING
app.post('/api/products', async (req, res) => {
  try {
    ...
    if (images && images.length > 0) {
      for (let base64Str of images) {
        // FIXED: Auto-clean mobile browser header prefix if present
        if (base64Str.startsWith("data:image")) {
          base64Str = base64Str.replace(/^data:image\/\w+;base64,/, "");
        }
        
        // Direct safe upload to Cloudinary using cleaned raw buffer string
    ...
    console.error("Transmission engine failed completely:", error);
    return res.status(500).json({ success: false, message: "Server capacity or credential block" });
  }
});
